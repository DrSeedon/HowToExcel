            ...
            void ParseData(ref int row)
            {
                ...
                for (;; row++)
                {
                    ...
                    for (int i = 2; i < BudgetData.ProcedureDatas.Count; i++)
                    {
                        ...
                        Write(sheet.Cells[i, 3], BudgetData.ProcedureDatas[i].OperationName);
                        Write(sheet.Cells[i, 4], BudgetData.ProcedureDatas[i].Amount.ToString());
                        Write(sheet.Cells[i, 5], BudgetData.ProcedureDatas[i].AccountBalance.ToString());
                    }
                }
            }
